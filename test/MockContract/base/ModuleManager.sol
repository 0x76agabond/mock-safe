...
abstract contract ModuleManager is SelfAuthorized, Executor, IModuleManager {
    ...
    function setupModules(address to, bytes memory data) internal {
        if (modules[SENTINEL_MODULES] != address(0)) ErrorMessage.revertWithError("GS100");
    ...
    function enableModule(address module) public override authorized {
        ...
        if (module == address(0) || module == SENTINEL_MODULES) ErrorMessage.revertWithError("GS101");
        // Module cannot be added twice.
        if (modules[module] != address(0)) ErrorMessage.revertWithError("GS102");
    ...
    function disableModule(address prevModule, address module) public override authorized {
        ...
        if (module == address(0) || module == SENTINEL_MODULES) ErrorMessage.revertWithError("GS101");
        if (modules[prevModule] != module) ErrorMessage.revertWithError("GS103");
